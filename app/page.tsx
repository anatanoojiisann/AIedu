    ...
    <main>
          ...
          <div className="glass-card rounded-xl p-4 text-cosmic-text">
            {visualLoading ? (
              ...
              <div className="space-y-3">
                {["Training panel", "Role-play scoring", "Analytics trends"].map((item, i) => <div key={item} className={`rounded-lg border border-cosmic-silver/40 p-3 ${i===1?"translate-x-3":""}`}>{item}</div>)}
              </div>
            )}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-8">
        <div className="grid gap-4 md:grid-cols-3">
          {[{title:"Go live in 24 hours",sub:"From workflow import to first manager report",icon:<Workflow size={16}/>},
          {title:"92% scenario completion",sub:"Verified role-play scoring",icon:<ChartNoAxesColumn size={16}/>},
          {title:"34% fewer onboarding errors",sub:"Measured on SOP checkpoints",icon:<ShieldCheck size={16}/>}].map((k) => (
